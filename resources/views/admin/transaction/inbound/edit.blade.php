<x-guest-layout>
    ...
    <div style="padding: 2rem 1rem; max-width: 1400px; margin: 0 auto;">
        ...
        <div style="display: flex; flex-direction: column; align-items: flex-start; margin-bottom: 3rem;">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item" style="color: black;"><a href="{{ route('admin.transaction.index', ['tab' => 'inbound']) }}">Transactions</a></li>
                    <li class="breadcrumb-item" style="color: black;"><a href="{{ route('admin.transaction.inbound.detail', $transaction->reference_number) }}">Purchase #{{ $transaction->reference_number }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page" style="color: #6366f1">Edit Purchase #{{ $transaction->reference_number }}</li>
                </ol>
            </nav>
            <h1 style="font-size: 2.5rem; font-weight: 700; color: #111827; margin: 0 0 0.75rem 0; text-align: center; width: 100%;">
                Edit Purchase #{{ $transaction->reference_number }}
            </h1>
        ...
        <div style="background: white; border-radius: 1rem; box-shadow: 0 10px 30px -8px rgba(0,0,0,0.12); border: 1px solid #e5e7eb; padding: 3rem 2.5rem; max-width: 900px; margin: 0 auto;">
            ...
            <form action="{{ route('admin.transaction.inbound.update', $transaction->reference_number) }}" method="POST" enctype="multipart/form-data" id="purchase-form">
                @csrf
                @method('PUT')
                ...
                <div style="display: flex; flex-direction: column; gap: 3.5rem; align-items: center;">
                    ...
                    <div style="width: 100%; max-width: 600px;">
                        ...
                        <select name="supplier_id" id="supplier_id" required disabled
                                ...
                                <div class="item-row" style="border:1px solid #e5e7eb; border-radius:0.9rem; padding:1.75rem; background:#fafafa; position:relative;">
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.75rem;">
                                        ...
                                        <div>
                                            ...
                                            <select name="items[{{$index}}][catalog_supplier_id]" class="catalog-supplier-select" required
                                                    data-current-value="{{ old("items.$index.catalog_supplier_id", $item->catalog_supplier_id ?? '') }}"
                                        ...
                                        <div>
                                            ...
                                            <select name="items[{{$index}}][catalog_id]" class="catalog-general-select" required
                                                    style="width:100%; padding:0.9rem; border:1px solid #d1d5db; border-radius:0.6rem; font-size:1rem; color:#111827;">
                                                <option value="">-- Select from Catalog --</option>
                                                @foreach ($catalogs as $catalog)
                                                    <option value="{{ $catalog->id }}" {{ old("items.$index.catalog_id", $item->catalog_id) == $catalog->id ? 'selected' : '' }}>
                                                        {{ $catalog->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                ...
                <div style="margin-top: 4rem; display: flex; gap: 1.5rem; justify-content: center;">
                    <a href="{{ route('admin.transaction.inbound.detail', $transaction->reference_number ?? '') }}"
    ...
    <script>
        ...
        // Attach listeners (qty, discount, catalog change)
        function attachRowListeners(row) {
            ...
            const supSelect = row.querySelector('.catalog-supplier-select');
            if (supSelect) {
                supSelect.addEventListener('change', function() {
                    ...
                    row.querySelector('.price-hidden').dataset.price1 = p1;
                    row.querySelector('.price-hidden').dataset.price2 = p2;

                    row.querySelector('.title1-label').textContent = title1Name + ' Qty';
                    row.querySelector('.title2-label').textContent = title2Name + ' Qty';
        ...
        function loadSupplierProducts(row, supplierId) {
            ...
            const currentVal = select.value || select.dataset.currentValue || '';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('.item-row').forEach(row => {
                // Attach all behaviors to existing rows
        ...
        document.getElementById('add-item-btn')?.addEventListener('click', () => {
            ...
            template.querySelectorAll('[name^="items[]"]').forEach(el => {
                let name = el.getAttribute('name').replace('items[]', `items[${itemIndex}]`);
