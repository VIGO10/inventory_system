    ...
    <div style="padding: 2rem 1rem; max-width: 1400px; margin: 0 auto;">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page" style="color: #6366f1">
                            {{ $supplier->prefix ? $supplier->prefix . ' ' : '' }}{{ $supplier->name }}
                        </li>
                    </ol>
                </nav>
                <h1 style="
                    ...
                    margin: 0;
                ">
                    Catalog for {{ $supplier->prefix ? $supplier->prefix . ' ' : '' }}{{ $supplier->name }}
                </h1>
                <p style="
                    margin-top: 0.5rem;
                    color: #6b7280;
                    font-size: 1rem;
                ">
                    Manage catalog items for {{ $supplier->prefix ? $supplier->prefix . ' ' : '' }}{{ $supplier->name }}
                </p>
            </div>

            <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: center; width: 100%;">
                <!-- Add New Product Button -->
                <a href="{{ route('admin.supplier.catalog.create', $supplier->slug) }}"
                   style="
                       background: #6366f1;
                       color: white;
                ...
                <!-- Search Form -->
                <form action="{{ route('admin.supplier.catalog.index', $supplier->slug) }}" method="GET" style="flex: 1; min-width: 300px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        @if($search)
                            <a href="{{ route('admin.supplier.catalog.index', $supplier->slug) }}?tab={{ request('tab', 'available') }}"
                            style="
                                color: #ef4444;
                                font-weight: 500;
                                text-decoration: none;
        ...
        <div style="margin-bottom: 1.5rem;">
            <div style="display: flex; border-bottom: 2px solid #e5e7eb;">
                <a href="{{ route('admin.supplier.catalog.index', [$supplier->slug, 'tab' => 'available']) }}"
                   style="
                       padding: 0.75rem 1.5rem;
                ...
                <a href="{{ route('admin.supplier.catalog.index', [$supplier->slug, 'tab' => 'not-available']) }}"
                   style="
                       padding: 0.75rem 1.5rem;
                ...
                onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.08)'">
                    ...
                    <div style="padding: 1.25rem;">
                        ...
                        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: 1rem;">
                            <a href="{{ route('admin.supplier.catalog.edit', [$supplier->slug, $item->id]) }}"
                               style="
                                   background: #3b82f6;
                                   color: white;
                                   padding: 0.5rem 1rem;
                            ...
                            <form action="{{ route('admin.supplier.catalog.toggle', [$supplier->slug, $item->id]) }}" method="POST" style="display: inline;">
                                @csrf
                                <button type="submit"
                                        style="
                            ...
                            <form action="{{ route('admin.supplier.catalog.delete', [$supplier->slug, $item->id]) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
